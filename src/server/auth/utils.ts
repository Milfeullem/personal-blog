...
import { getTime, localTime } from '@/libs/dayjsUtil';
...
export const verifyJWT = async (c: Context) =>
  new Promise((resolve) => {
    passport.authenticate('jwt', { session: false }, async (err: any, user: AuthItem) => {
      ...
      resolve(true);
    })(c.req.raw, (c.res as any).raw, (err: Error) => {
 ...
 * @param handler
 */
export const createAuthenticatedHandler = (handler: any) =>
