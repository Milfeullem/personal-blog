...
import { getDayjs, getTime, localTime } from './dayjsUtil';
...
export const generateAccessToken = (user: AuthItem) => {
  const tokenExpiry: DurationUnitsObjectType =
    typeof authConfig.tokenExpiry === 'number'
...
export const setAccessToken = async (token: string) => {
  ...
  await setCookie(options.name, token, omit(options, ['name']));
};
