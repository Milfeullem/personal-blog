...
const TagInput = React.forwardRef<HTMLInputElement, TagInputProps>((props, ref) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (addOnPaste && newValue.includes(delimiter)) {
      ...
      splitValues.forEach((value) => {
        ...
        if (
          restrictTagsToAutocompleteOptions &&
          !autocompleteOptions?.some((option: Tag) => option.text === newTagText)
        ) {
        ...
        if (allowDuplicates || !tags.some((tag: Tag) => tag.text === newTagText)) {
  ...
  const handleInputBlur = (event: React.FocusEvent<HTMLInputElement>) => {
    if (addTagsOnBlur && inputValue.trim()) {
      ...
      if (
        (allowDuplicates || !tags.some((tag: Tag) => tag.text === newTagText)) &&
      ...
      switch (e.key) {
        case 'Delete':
          if (activeTagIndex !== null) {
            ...
            setTags(newTags);
            setActiveTagIndex((prev: number | null) =>
        ...
        case 'Backspace':
          if (activeTagIndex !== null) {
            ...
            newTags.splice(activeTagIndex, 1);
            setTags(newTags);
            setActiveTagIndex((prev: number | null) => (prev! === 0 ? null : prev! - 1));
            setTagCount((prevTagCount) => prevTagCount - 1);
            onTagRemove?.(tags[activeTagIndex].text);
          }
          break;
        case 'ArrowRight':
          ...
          if (activeTagIndex === null) {
            ...
            setActiveTagIndex((prev: number | null) => (prev! + 1 >= tags.length ? 0 : prev! + 1));
          }
          break;
        case 'ArrowLeft':
          e.preventDefault();
          if (activeTagIndex === null) {
            ...
            setActiveTagIndex((prev: number | null) => (prev! === 0 ? tags.length - 1 : prev! - 1));
          }
          break;
        case 'Home':
          e.preventDefault();
  ...
  const removeTag = (idToRemove: string) => {
    setTags(tags.filter((tag: Tag) => tag.id !== idToRemove));
    onTagRemove?.(tags.find((tag: Tag) => tag.id === idToRemove)?.text || '');
    setTagCount((prevTagCount) => prevTagCount - 1);
  };
  ...
  const filteredAutocompleteOptions = useMemo(() => {
    ...
    return (autocompleteOptions || []).filter((option: Tag) =>
