...
function DesktopToc({ serialized }: { serialized: MdxHydrateProps['serialized'] }) {
  if (isNil(serialized.scope?.toc) || serialized.scope?.toc.length < 1) {
    return null;
  }
...
function MobileToc({ serialized }: { serialized: MdxHydrateProps['serialized'] }) {
