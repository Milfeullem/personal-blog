...
.tools {
  @apply flex justify-between my-5 mx-3 w-full max-h-12;
}
```
...
.list {
    ...
    & .item {
        @apply rounded-md mb-8 flex flex-col
        duration-300 backdrop-blur-md
        drop-shadow-[5px_5px_5px_rgba(0,0,0,0.35)];
        ...
        &::before {
            ...
            @apply opacity-0 rounded-md  overflow-hidden duration-300
            absolute top-0 left-0 w-full h-full bg-[image:var(--bg-img)]
            z-[-1] bg-center bg-cover;
...
export const PostActionButtons: FC<{ id: string }> = ({ id }) => {
  return (
    <div className="flex items-center [&>time]:ml-2">
      <PostDelete id={id} />
    </div>
  );
};
...
.modalWrapper {
  @apply w-full h-full flex-auto;
}

.modalContent {
  @apply overflow-y-auto max-h-[80vh] w-full px-3;
}
```
...
.item {
  @apply bg-white/80 flex-auto w-full drop-shadow-lg rounded-md flex flex-col;
}
```
...
.details {
  @apply border p-2 overflow-hidden;

  & > .content {
    @apply overflow-hidden px-2 py-3;
  }
}
```
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="请输入内容" {...field} className="min-h-80" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
.item {
  @apply flex flex-auto flex-col space-y-6  bg-white/90  min-w-full lg:min-w-[60%] !max-w-full px-5 py-3 rounded-md;
}
...
/* src/app/(pages)/posts/[item]/page.module.css */
.item {
  @apply bg-white/80 dark:bg-zinc-800/80 flex-auto w-full drop-shadow-lg rounded-md flex flex-col;

  & > .thumb {
    @apply relative w-full h-36 md:h-48 lg:h-64 block;
  }

  & > .thumb img {
    @apply rounded-tl-md rounded-tr-md opacity-60;
  }
