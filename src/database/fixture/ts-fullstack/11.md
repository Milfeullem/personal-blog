...
.item {
  @apply flex flex-col w-auto space-y-8 bg-white/90 dark:bg-black/50 
   !w-full lg:!w-auto mx-auto px-5 py-9 rounded-md shadow-md mt-28;
}
```
...
export const HeaderTools: FC<{ isMobile?: boolean }> = ({ isMobile = false }) => {
  ...
  return (
    <div className={$styles.tools}>
      <div className="flex">{auth && <PostCreateButton iconBtn={isMobile} />}</div>
      <ShadcnThemeSetting />
    </div>
  );
};
        ...
        <div className={cn($styles.user)}>
            {isNil(auth) ? (
                ...
                <DropdownMenu modal={false}>
                    ...
                    <DropdownMenuContent
                        align="center"
                        className="w-56 text-center text-stone-500"
                    >
...
.user .avatar {
  @apply w-10 h-10 block rounded-full p-1 bg-white cursor-pointer duration-200 transition-shadow;
  ...
  &:hover {
    @apply shadow-amber-400  shadow-nylg;
  }
}
...
.user > .authBtns > .authBtn {
  @apply flex items-center justify-center px-4 py-2 
            rounded-full text-sm font-medium transition-all duration-300 border border-transparent
