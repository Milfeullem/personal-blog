...
.logo {
  @apply flex flex-none items-center max-w-28;
}
...
export const Header: FC = () => {
  ...
  return (
    <header
      className={cn($styles.header, 'page-container', {
...
.header {
  @apply fixed top-0 w-full z-40  mt-0 bg-white/40 dark:bg-zinc-900/10 transition-[background-color] ease-in-out duration-300;
}

.header-scrolled {
  @apply border-b border-gray-200/40 dark:border-gray-800/40  shadow-md bg-white/80 dark:bg-zinc-900/50 backdrop-blur-md;
}

.header > .container {
  @apply flex h-14 w-full justify-between flex-row mb-0 transition-[height] ease-in-out duration-300;
}
...
.item {
  @apply flex flex-auto flex-col space-y-6  bg-white/90 dark:bg-black/80  min-w-full lg:min-w-[60%] !max-w-full px-5 py-3 rounded-md;
}
```
...
.mdx-preview {
  /* 链接样式 */
  a {
    @apply text-blue-800/70 hover:text-blue-700/80 dark:text-blue-300/70 dark:hover:text-blue-200/80 mx-0.5 transition-colors duration-200;
  }
  /* ... */
}
```
...
.mdx-preview {
  ...
  h6 {
    ...
    & > a {
      @apply after:text-slate-700 dark:after:text-slate-300;
    }
...
.container {
  ...
  & :global(.wmde-markdown-var:has(.w-md-editor-text-input:disabled)) {
    ...
    &::before {
      @apply content-[''] absolute inset-0 z-[99] pointer-events-none rounded-[3px] bg-zinc-300/50 dark:bg-zinc-900/50 backdrop-blur-[2px];
    }
