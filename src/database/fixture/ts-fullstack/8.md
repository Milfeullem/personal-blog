...
.item {
  ...
  /* 加一个tw-flex-auto */
  & > .content {
    @apply p-3 flex flex-col flex-auto;
  }
}
```
...
export const Button: FC<{ iconBtn?: boolean }> = ({ iconBtn }) => {
  ...
  return (
    <CNButton
      asChild
      className={cn('ml-auto justify-end', {
...
.side {
  @apply fixed flex flex-col h-screen w-60 z-[999] 
           bg-[rgb(255_255_255/0.8)] dark:bg-[rgba(24_24_27/0.9)] bg-opacity-90 backdrop-blur-sm
           transition-transform duration-300 ease-out  -translate-x-full;

  & > .top {
    @apply flex flex-none items-center justify-between px-4 h-10 shadow-[rgba(0_0_0/0.1)_0px_1px_2px_0px];
  }
...
.modal.open {
  @apply block fixed inset-0 bg-black/50 z-50;
}
```
...
/* ... */
& > .footer {
  @apply py-3 flex justify-between text-muted-foreground flex-wrap space-y-3 md:space-y-0;
  /* ... */
}
```
...
@layer utilities {
  ...
  .transparent-scrollbar {
    ...
    &::-webkit-scrollbar-thumb {
      @apply bg-gray-300 dark:bg-zinc-600 rounded;
    }
...
.desktopToc {
  @apply w-56 flex-shrink-0 sticky top-16 self-start;
}

.mobileToc {
  @apply fixed flex items-center right-0 top-16 z-10 top-[30%] overflow-hidden 
  ...
  & > .mobileTocMain {
    @apply h-full w-56 bg-[rgb(255_255_255/0.8)] dark:bg-[rgba(24_24_27/0.9)] backdrop-blur-sm drop-shadow-sm
               shadow-lg  rounded-tl-lg rounded-bl-lg py-2;

    & > .mobileTocTitle {
      @apply flex items-center py-2 px-4 justify-between;
    }
...
```css
.container {
  @apply w-full flex gap-8 relative;
...
由于`drop-shadow-*`样式会对`fixed`的容器范围产生影响，所以请务必把文章详情页样式中的`drop-shadow-lg`改掉或去掉

```css
/* src/app/(pages)/posts/[item]/page.module.css */
.item {
  @apply bg-white/80 dark:bg-zinc-800/80 flex-auto w-full shadow-lg rounded-md flex flex-col;
  /* ... */
}
```
...
<div className={clsx($styles.title, 'hover')}>
...
.page-blank {
  @apply bg-white/80 dark:bg-black/50 backdrop-blur-sm h-full w-full drop-shadow-lg rounded-md flex justify-center items-center;
}
```
  ...
  return (
    <Theme>
      <div className={$styles.layout}>
        <Header />
        <div className="page-container">
          <div className="page-blank  flex flex-col space-y-4">
            <h2>糟糕!服务器挂了...</h2>
            <p>
              错误信息: {error.message} |
              <b className="ml-3">
...
const PageSkeleton: FC = () => (
  <div className="w-full justify-center space-y-7">
    ...
    <div className="flex flex-col space-y-5">
      <Skeleton className="h-52 w-full backdrop-blur-md" />
      <div className="flex h-16 w-full justify-between space-x-20">
        <Skeleton className="w-1/3 flex-none bg-gray-950/30 backdrop-blur-sm" />
        <Skeleton className="flex-auto bg-gray-950/30 backdrop-blur-sm" />
      </div>
...
.editor {
  & :global(.cm-scroller) > div[contenteditable='false'] {
    ...
    &::before {
      ...
      @apply absolute top-0 left-0 w-full h-full bg-white/45 z-10;
    }
  }
}
```
