...
- ``为编写的所有tailwind类的前缀，比如`bg-black`。这是为了防止与antd、bootstrap等其他组件库冲突
- `darkMode`设置为`['selector', '[class~="dark"]']`用于手动切换暗黑模式。这样设置，在`html`节点上有`dark`或`dark`类时，就会切换到暗黑模式
...
@layer base {
    ...
    * {
        /* 注意,shadcn/ui生成该样式时,因为还没有在`components.json`中加上``前缀,所以该样式是不带有tw-前缀的.请自行加上tw-前缀 */
        @apply border-border;
    }
   /* 以下为自定义样式 */
    html,
...
.block {
  @apply flex flex-col !flex-none w-full space-y-4 p-5 items-center justify-center 
...
.layout {
  /* 设置全局背景图片 */
  @apply bg-fixed bg-cover bg-no-repeat bg-center min-h-full w-full flex p-0 m-0 flex-col
    bg-[url(../styles/images/bg-light.png)] 
...
.layout {
  @apply flex flex-auto flex-wrap items-center justify-center bg-[url(../styles/images/bg-light.png)];
}
```
